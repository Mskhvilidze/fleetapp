...
public interface SupplierRepository extends JpaRepository<Supplier, Integer> {
    @Transactional
    @Modifying
    @Query(nativeQuery = true, value = "delete from fleetappdb.supplier where id = ?")
    int deleteState(int id);

    @Transactional
    @Modifying
    @Query(nativeQuery = true,
            value = "UPDATE fleetappdb.supplier SET countryId = :countryId, stateId = :stateId, name = :name, details = :details, " +
